...
results_1 = results[:half]

rateable_pool = create_rateable_pool_from_video_ids(video_ids)
rateable_pool = process_unweighted_results(rateable_pool, results_1)
...
with open('first_half_split_ratings.json', 'w') as f:
    json.dump(relevant_ratings, f, indent=4)

results_2 = results[half:]

length = len(results_2)

for i in range(1,9):
    a = round(i*0.125*length)
    split_results = results_2[:a]
    rateable_pool = create_rateable_pool_from_video_ids(video_ids)
    ...
    with open('half_split_ratings' + str(i) + '.json', 'w') as f:
        json.dump(relevant_ratings, f, indent=4)
