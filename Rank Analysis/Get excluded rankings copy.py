...
responses_path = Path("feb4_expert_responses.json")
...
for i in range(12):
    indexes = list(set(a) - set([i]))
    #resultset = [value for key, value in results_per_user.items() if key not in i]

    # get all results except those done by specific user

    for number in indexes:
        excluded_results += results_per_user[number]
    #results = results_per_user[i+1] + results_per_user[i+2] + results_per_user[i+3] + results_per_user[i+4]+ results_per_user[i+5]+ results_per_user[i+6]+ results_per_user[i+7]+ results_per_user[i+8]+ results_per_user[i+9]

    video_ids = config[0]['cases']
    rateable_pool = create_rateable_pool_from_video_ids(video_ids)
    rateable_pool = process_unweighted_results(rateable_pool, excluded_results)
...
import json
with open('ratings_excluded_users.json', 'w') as f:
    json.dump(ratings_all_users,f,indent=4)

# rateable_pool_users now has lines 0 to 9 of user excluded ratings, + line 10 of total ratings
